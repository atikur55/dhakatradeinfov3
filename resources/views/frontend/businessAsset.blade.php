                                    ...
                                    <div class="row">
                                        <div class="col-5">
                                            <div class="categories-details-img">
                                                <img class="img-fluid" src="{{ asset('uploads/category') }}/{{ $category->image??'photo.jpg' }}" alt="">
                                            </div>
                                        </div>
                                        <div class="col-7">
                                            <div class="categories-details">
                                                <!--<h6><a href="{{ route('category.assets',['name'=>$category->category_name]) }}">{{ $category->category_name??'' }}</a></h6>-->
                                                <h6><a href="{{url('category')}}/{{$category->category_name}}/{{$category->business_id}}">{{ $category->category_name??''}}</a></h6>
                                                @php
                                                    $sub_categories = App\Models\SubCategory::where('category_id',$category->id)->where('status',0)->take(5)->get();
                                                @endphp
                                                <ul>
                                                    @forelse ($sub_categories as $subcategory)
                                                        {{-- <li>{{ $product->title??'' }}</li> --}}
                                                        <a href="{{ url('subcategory_product') }}/{{ $subcategory->id }}"><li>{{ $subcategory->title??'' }}</li></a>
                                                    @empty
                                                        <li>No Data Found</li>
                                                    @endforelse
                                                </ul>
                                                <div class="categories-viewdetails">
                                                    <a href="{{ route('allAssetsUnCat',['id' => $category->id]) }}">View Details</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="product-list-bg">
                ...
                <div class="row px-3">
                    @forelse ($products as $product)
                                    ...
                                    @endphp<div class="col-sm-6 col-md-3 col-lg-2 mb-4">
