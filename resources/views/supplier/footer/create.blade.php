...
Footer
@endsection
@section('supplier_fFooterooter_info')
 menu-item-active
@endsection
@section('css')
							   	...
							   	<ul class="nav nav-bold nav-pills ml-auto">
								    <li class="nav-item">
					                    ...
					                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalLong">
					                        <i class="flaticon2-plus-1 icon-lg"></i> Add New Footer
					                    </button>
					                        ...
					                        <div class="modal-dialog modal-lg" role="document">
					                            <div class="modal-content">
					                                <div class="modal-header">
					                                    <h5 class="modal-title" id="exampleModalLabel">Create New Footer</h5>
					                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
					                                        <i aria-hidden="true" class="ki ki-close"></i>
					                                    </button>
					                                </div>
					                                <div class="modal-body">
					                                   	<form action="{{ route('supplier.footer.post') }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" class="form-control form-control-solid" name="email" placeholder="Email Address"/>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <textarea name="address" id="" class="form-control" cols="3" rows="3" placeholder="Company Address"></textarea>
                                                            <span class="form-text text-muted">Please enter your Email Address</span>
                                                            @error('email')
                                                            <div class="alert alert-danger">
                                                                    <strong>{{$message}}</strong>
										...
										<table id="usertable" class="table table-bordered table-striped">
											...
											<tbody>
												...
												<tr>
                                                    ...
                                                    <td>{{ $footer->phone }}</td>
                                                    <td>{{ $footer->email }}</td>
													<td><img src="{{asset('uploads/supplier/footer')}}/{{$footer->image}}" alt="" width="80px;"></td>
                                                    <td>
														@if($footer->status == 1)
														<a href="#statusModal{{$footer->id}}" data-toggle="modal" class="btn btn-success"  ><i class="fas fa-toggle-on icon-md"></i>Active
					                                    </a>
														@else
														<a href="#statusModal{{$footer->id}}" data-toggle="modal" class="btn btn-danger"  ><i class="fas fa-toggle-off icon-md"></i></i> InActive
					                                    </a>
														@endif
													</td>
													<td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            	@if($footer->status==1)
                <a href="{{route('supplier.footer.status', ['id' => $footer->id])}}" class="btn btn-danger">Deactive</a>
                @else
                <a href="{{route('supplier.footer.status', ['id' => $footer->id])}}" class="btn btn-success">Active</a>
                @endif
...
<div class="modal fade" id="deleteModal{{$footer->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="{{route('supplier.footer.delete', ['id' => $footer->id])}}" class="btn btn-danger">Delete</a>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
            </div>
        </div>
    </div>
...
<div class="modal fade " id="editModal{{$footer->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('supplier.footer.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label>Phone Number:</label>
                        <input type="text" class="form-control form-control-solid" name="phone" placeholder="Phone Number" value="{{ $footer->phone }}"/>
                        <span class="form-text text-muted">Please enter your Phone Number</span>
                        @error('phone')
                        <div class="alert alert-danger">
                                <strong>{{$message}}</strong>
                    ...
                    <div class="form-group">
                        <label>Email Address</label>
                        <input type="text" class="form-control form-control-solid" name="email" placeholder="Email Address" value="{{ $footer->email }}"/>
                        <span class="form-text text-muted">Please enter your Email Address</span>
                        @error('email')
                        <div class="alert alert-danger">
                                <strong>{{$message}}</strong>
                    ...
                    <div class="form-group">
                        <label>Company Address</label>
                        <textarea name="address" id="" class="form-control" cols="3" rows="3" placeholder="Company Address">{{ $footer->address }}</textarea>
                        <span class="form-text text-muted">Please enter your Email Address</span>
                        @error('email')
                        <div class="alert alert-danger">
                                <strong>{{$message}}</strong>
                    ...
                    <div class="form-group">
                            <label>Logo </label>
                            <input type="hidden" class="form-control form-control-solid" name="id"  value="{{ $footer->id }}"/>
                            <div></div>
                            <div class="custom-file">
                                <input type="file" class="custom-file-input" id="customFile" name="image" id="profile-img" onchange="preview_image(event)"/>
                                <label class="custom-file-label" for="customFile">Choose file</label>
                                <img src="{{ asset('uploads/supplier/footer') }}/{{ $footer->image }}" id="output_image" width="200px" style="padding-top: 5px;" />
                            </div>
                    </div>
                    @error('image')
                    <div class="alert alert-danger">
